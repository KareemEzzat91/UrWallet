...
class FinancialInboxAdapter(
    ...
    inner class ViewHolder(
        ...
        fun bind(item: FinancialEvent) {
            ...
            // 5. Possible Duplicate Warning Banner
            val isPossibleDuplicate = item.matchStatus == DuplicateMatchStatus.POSSIBLE_MATCH
            binding.layoutDuplicateWarning.isVisible = isPossibleDuplicate
            if (isPossibleDuplicate) {
                binding.tvDuplicateWarning.text = context.getString(R.string.badge_possible_duplicate)
            }

            // 6. Action buttons (Visible only for PENDING items)
            val isPending = item.status == InboxStatus.PENDING
            binding.layoutActionButtons.isVisible = isPending
